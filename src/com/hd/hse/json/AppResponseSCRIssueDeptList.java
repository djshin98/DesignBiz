...
public class AppResponseSCRIssueDeptList extends JSONWrapper{
	public static final int ID = 63;
	public static final String NAME = "AppResponseSCRIssueDeptList";
	...
	public ArrayList<AppDepartment>		departments;	//협력업체 목록
	public String		language;	//모바일 사용자가 선택한 언어(한국어="ko",영어="en")

	public void copy( AppResponseSCRIssueDeptList d ){
	...
	public JSONObject encoding(){
		...
		try {
			...
			obj.put("departments", new JSONArray() );
			if( departments != null ){
				JSONArray array = obj.getJSONArray("departments");
				for(int i = 0 ; i < departments.size() ; i++ ){
					array.put( ((AppDepartment)(departments.get(i))).encoding() );
				}
			}
	...
	public static AppResponseSCRIssueDeptList decoding(JSONObject obj) {
		AppResponseSCRIssueDeptList wrapper = new AppResponseSCRIssueDeptList();
		try {
			...
			if( obj.has("departments") ){
				wrapper.departments = new ArrayList<AppDepartment>();
				JSONArray array = obj.getJSONArray( "departments" );
				for( int i = 0 ; i < array.length() ; i++ ){
					wrapper.departments.add( AppDepartment.decoding( array.getJSONObject(i)));
				}
			} 
	...
	public void print(String prefix){
		...
		System.out.println(prefix+"departments : ");
		for(int i = 0 ; i < departments.size() ; i++ ){
			((AppDepartment)departments.get(i)).print("	"+prefix);
		}
		if( language != null ){
	...
	public JSONObject toMessage(){
		JSONObject obj = new JSONObject();
		try {
			obj.put("requestObjectName", AppResponseSCRIssueDeptList.NAME);
