...
public class AppResponseSCREmployeeList extends JSONWrapper{
	public static final int ID = 66;
	public static final String NAME = "AppResponseSCREmployeeList";
	...
	public ArrayList<AppOrganItem>		organs;	//조직 목록
	public String		language;	//모바일 사용자가 선택한 언어(한국어="ko",영어="en")

	public void copy( AppResponseSCREmployeeList d ){
		this.requestKey = d.requestKey;
		this.result = d.result;
		this.resultMessage = d.resultMessage;
		this.checkDate = d.checkDate;
		this.organs = d.organs;
	...
	public JSONObject encoding(){
		...
		try {
			...
			obj.put("organs", new JSONArray() );
			if( organs != null ){
				JSONArray array = obj.getJSONArray("organs");
				for(int i = 0 ; i < organs.size() ; i++ ){
					array.put( ((AppOrganItem)(organs.get(i))).encoding() );
				}
			}
	...
	public static AppResponseSCREmployeeList decoding(JSONObject obj) {
		AppResponseSCREmployeeList wrapper = new AppResponseSCREmployeeList();
		try {
			...
			if( obj.has("organs") ){
				wrapper.organs = new ArrayList<AppOrganItem>();
				JSONArray array = obj.getJSONArray( "organs" );
				for( int i = 0 ; i < array.length() ; i++ ){
					wrapper.organs.add( AppOrganItem.decoding( array.getJSONObject(i)));
				}
			} 
	...
	public void print(String prefix){
		...
		System.out.println(prefix+"organs : ");
		for(int i = 0 ; i < organs.size() ; i++ ){
			((AppOrganItem)organs.get(i)).print("	"+prefix);
		}
		if( language != null ){
	...
	public JSONObject toMessage(){
		JSONObject obj = new JSONObject();
		try {
			obj.put("requestObjectName", AppResponseSCREmployeeList.NAME);
