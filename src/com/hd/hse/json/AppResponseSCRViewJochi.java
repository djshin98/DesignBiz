...
public class AppResponseSCRViewJochi extends JSONWrapper{
	public static final int ID = 87;
	public static final String NAME = "AppResponseSCRViewJochi";
	...
	public ArrayList<AppNCRResult>		ncrList;	//부적합 보고서 조치결과 목록
	...
	public JSONObject encoding(){
		...
		try {
			...
			obj.put("ncrList", new JSONArray() );
			if( ncrList != null ){
				JSONArray array = obj.getJSONArray("ncrList");
				for(int i = 0 ; i < ncrList.size() ; i++ ){
					array.put( ((AppNCRResult)(ncrList.get(i))).encoding() );
				}
			}
	...
	public static AppResponseSCRViewJochi decoding(JSONObject obj) {
		AppResponseSCRViewJochi wrapper = new AppResponseSCRViewJochi();
		try {
			...
			if( obj.has("ncrList") ){
				wrapper.ncrList = new ArrayList<AppNCRResult>();
				JSONArray array = obj.getJSONArray( "ncrList" );
				for( int i = 0 ; i < array.length() ; i++ ){
					wrapper.ncrList.add( AppNCRResult.decoding( array.getJSONObject(i)));
				}
			} 
	...
	public void print(String prefix){
		...
		System.out.println(prefix+"ncrList : ");
		for(int i = 0 ; i < ncrList.size() ; i++ ){
			((AppNCRResult)ncrList.get(i)).print("	"+prefix);
		}
		if( userid != null ){
	...
	public JSONObject toMessage(){
		JSONObject obj = new JSONObject();
		try {
			obj.put("requestObjectName", AppResponseSCRViewJochi.NAME);
