...
public class AppResponseLawFileList extends JSONWrapper{
	public static final int ID = 29;
	public static final String NAME = "AppResponseLawFileList";
	...
	public ArrayList<AppLawFile>		files;	//법률 파일 목록
	public String		language;	//모바일 사용자가 선택한 언어(한국어="ko",영어="en")

	public void copy( AppResponseLawFileList d ){
	...
	public JSONObject encoding(){
		...
		try {
			...
			obj.put("files", new JSONArray() );
			if( files != null ){
				JSONArray array = obj.getJSONArray("files");
				for(int i = 0 ; i < files.size() ; i++ ){
					array.put( ((AppLawFile)(files.get(i))).encoding() );
				}
			}
	...
	public static AppResponseLawFileList decoding(JSONObject obj) {
		AppResponseLawFileList wrapper = new AppResponseLawFileList();
		try {
			...
			if( obj.has("files") ){
				wrapper.files = new ArrayList<AppLawFile>();
				JSONArray array = obj.getJSONArray( "files" );
				for( int i = 0 ; i < array.length() ; i++ ){
					wrapper.files.add( AppLawFile.decoding( array.getJSONObject(i)));
				}
			} 
	...
	public void print(String prefix){
		...
		for(int i = 0 ; i < files.size() ; i++ ){
			((AppLawFile)files.get(i)).print("	"+prefix);
		}
		if( language != null ){
	...
	public JSONObject toMessage(){
		JSONObject obj = new JSONObject();
		try {
			obj.put("requestObjectName", AppResponseLawFileList.NAME);
