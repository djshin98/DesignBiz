...
public class AppResponseSCRList extends JSONWrapper{
	public static final int ID = 59;
	public static final String NAME = "AppResponseSCRList";
	...
	public ArrayList<AppNCRSimple>		ncrList;	//NCR 목록
	public String		language;	//모바일 사용자가 선택한 언어(한국어="ko",영어="en")

	public void copy( AppResponseSCRList d ){
		...
		try {
			...
			if( ncrList != null ){
				...
				for(int i = 0 ; i < ncrList.size() ; i++ ){
					array.put( ((AppNCRSimple)(ncrList.get(i))).encoding() );
				}
			}
	...
	public static AppResponseSCRList decoding(JSONObject obj) {
		AppResponseSCRList wrapper = new AppResponseSCRList();
		try {
			...
			if( obj.has("ncrList") ){
				wrapper.ncrList = new ArrayList<AppNCRSimple>();
				JSONArray array = obj.getJSONArray( "ncrList" );
				for( int i = 0 ; i < array.length() ; i++ ){
					wrapper.ncrList.add( AppNCRSimple.decoding( array.getJSONObject(i)));
				}
			} 
	...
	public void print(String prefix){
		...
		for(int i = 0 ; i < ncrList.size() ; i++ ){
			((AppNCRSimple)ncrList.get(i)).print("	"+prefix);
		}
		if( language != null ){
	...
	public JSONObject toMessage(){
		JSONObject obj = new JSONObject();
		try {
			obj.put("requestObjectName", AppResponseSCRList.NAME);
