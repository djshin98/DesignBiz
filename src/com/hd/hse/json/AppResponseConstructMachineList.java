...
public class AppResponseConstructMachineList extends JSONWrapper{
	public static final int ID = 23;
	public static final String NAME = "AppResponseConstructMachineList";
	public String		requestKey;	//요청 키
	public String		result;	//요청 처리 결과값
	public String		resultMessage;	//처리 오류 상세 메시지
	public Timestamp		checkDate;	//시간 정보
	public ArrayList<AppConstructMachineInfo>		constructMachineInfos;	//
	public String		language;	//모바일 사용자가 선택한 언어(한국어="ko",영어="en")

	public void copy( AppResponseConstructMachineList d ){
		...
		try {
			...
			if( constructMachineInfos != null ){
				...
				for(int i = 0 ; i < constructMachineInfos.size() ; i++ ){
					array.put( ((AppConstructMachineInfo)(constructMachineInfos.get(i))).encoding() );
				}
			}
	...
	public static AppResponseConstructMachineList decoding(JSONObject obj) {
		AppResponseConstructMachineList wrapper = new AppResponseConstructMachineList();
		try {
			...
			if( obj.has("constructMachineInfos") ){
				wrapper.constructMachineInfos = new ArrayList<AppConstructMachineInfo>();
				JSONArray array = obj.getJSONArray( "constructMachineInfos" );
				for( int i = 0 ; i < array.length() ; i++ ){
					wrapper.constructMachineInfos.add( AppConstructMachineInfo.decoding( array.getJSONObject(i)));
				}
			} 
	...
	public void print(String prefix){
		...
		for(int i = 0 ; i < constructMachineInfos.size() ; i++ ){
			((AppConstructMachineInfo)constructMachineInfos.get(i)).print("	"+prefix);
		}
		if( language != null ){
	...
	public JSONObject toMessage(){
		JSONObject obj = new JSONObject();
		try {
			obj.put("requestObjectName", AppResponseConstructMachineList.NAME);
