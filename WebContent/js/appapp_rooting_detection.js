//앱 루팅 탐지
/*******************************************************************************************************************************************************************************
Mobile HSE , 2015.09 RequestAppRootingDetectReport 앱 루팅 탐지
*******************************************************************************************************************************************************************************/
var RequestAppRootingDetectReport = function(){
	this.prop = {
		requestObjectName : 'AppRequestAppRootingDetectReport',
		requestObject :{ 
			requestKey : '',			//요청 키
	...
	//RequestAppRootingDetectReport 오브젝트 초기화
	this.init = function(){
		...
		this.prop.requestObject.report='';
		this.prop.requestObject.language='';

    };
    //RequestAppRootingDetectReport 오브젝트를 json string로 변환
    this.toString = function(){
    	return JSON.stringify( this.prop );
    };
    this.request = function( onSuccessResponse , onFailResponse ){
    	//RequestAppRootingDetectReport request Message를 서버에 요청 하고, ResponseAppRootingDetectReport Message가 recevedData로 리턴받는다.
    	M.net.http.send({
			server:	'mhse',
		    ...
		    onSuccess:	function(receivedData, setting) {
		    	...
		    	if( receivedData.result == 'OK' ){
		    		...
		    		}else{ //RequestAppRootingDetectReport 요청이 서버에 전송되었고, 처리중 실패(처리 부적합)로 판단하여 리턴 
		    			onFailResponse(receivedData, setting);
		    		}
		    	}
		    },
		    "onError":	function(errorCode, errorMessage, setting) { //RequestAppRootingDetectReport 요청이 서버와 연결중 네트워크 또는 기타 오류...
